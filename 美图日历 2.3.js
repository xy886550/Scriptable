...
const elementSpacing = 1  //元素行间距
const refreshInterval = 20 //刷新时间(分钟)
//const showDlTq = false     //是否显示电量/天气

// 时间字体显示设置
const NowFont = "AvenirNext-Regular"
const NowSize = 35 //时间字体大小
const NowColor = new Color("B0E0E6")
...
if (colorMode) {
  ...
  const img = await getImageByUrl(img_url, `x.yi`, false)
...
async function createWidget() {
  ...
  if (NowFont == "" || NowFont == null) {
   Now.font = Font.regularSystemFont(NowSize)
...
//   }
  ...
  widget.refreshAfterDate = new Date(Date.now() + 1000 * 60 * refreshInterval)
