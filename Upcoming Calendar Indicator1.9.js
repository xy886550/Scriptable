...
- 增加了给日期首字母文本着色的选项
- 更好地将日期与单个事件的点对齐
- 删除了一些不需要的额外堆栈，以帮助提高RAM使用率
- 增加了优化的数组处理方法
- 增加了prevMonth和nextMonth标志，如果开始和结束周有空位，允许显示上个月和下个月的日期
- 增加了showDatesBeforeAfter的设置问题
- 优化了右侧日历视图中每个日期所使用的URL，以便它们在日历应用程序中打开适当的日期
- 修改了右侧日期堆叠的大小，以便更好地适应较大的月份，如2021年5月
- 修改了事件列表视图，如果事件是全日的，点击该事件就会进入适当的日期
--------------------------*/
/*
####################
####################
...
//日历名称包含在下面的cal变量中，以便在日历事件列表和月视图的指标中显示它们。这些必须用单引号或双引号括起来。(这由设置问题来处理)
...
//如果你想在左侧事件列表中的事件之间有额外的间隔，如果少于5个，将允许动态间隔的标志设置为true。如果你不想要动态间隔，请设置为false。
...
//如果你想让星期一成为月视图中一周的开始，就把monWeekStart的标志设置为true。如果你希望周日是一周的开始，那么就设置为false。
...
//将useBackgroundColor标志设置为true，以利用下面的backgroundColor变量。这可以根据你的喜好来设置。

const useBackgroundColor = settings.useBackgroundColor

//backgroundColor below is setup as darkGray by default but can be changed to hex as well.下面的backgroundColor默认设置为darkGray，但也可以改成十六进制。

if(settings.useBackgroundColor){const backgroundColor = new Color(settings.backgroundColor)}

//shows the dates before and after the current month and 显示当前月份之前和之后的日期，以及
...
//useTransparency是在初始化问题中设置的。这决定了是否应该使用透明度设置。
...
//useEventShadow在初始化问题中被设置。这决定了事件名称后面是否应该有一个阴影，以帮助在某些情况下的可读性。
...
//eventFontSize设置在左侧视图中显示的事件标题的大小。默认为11。
...
//showCurrentAllDayEvents可以显示今天发生的、被设置为全天的事件。
...
//useBaseTextColor是在初始化问题中设置的。这决定了基础文本是否应该有一个不同于默认的颜色（浅色模式下为黑色，深色模式下为白色），以帮助渲染，这取决于你的设置（主要是如果你使用透明度和有一张导致文本无法阅读的照片）。
...
//use24hrTime是一个设置问题，它允许用户以24小时格式而不是12小时格式显示事件时间。
...
//useSundayColor是一个设置问题，它允许在月视图中为周日日期使用用户定义的颜色。
...
//useSaturdayColor是一个设置问题，允许在月视图中为星期六的日期使用用户定义的颜色。
...
//useLineSeparator是一个设置问题，允许用户选择是否在日期名称和日历的其他部分之间显示一条线。
...
//heatMapEnabled有点不言自明，但这个标志可以启用热图功能。
let heatMapColor,heatMapMax,heatMapEnabled = settings.heatMapEnabled
if(heatMapEnabled){
  //heatMapColor determines the base color to use as the heat map color/heatMapColor决定了用作热图颜色的基础颜色。
  heatMapColor = settings.heatMapColor
  
  //heatMapMax determines the number of events in a given day to show as the full color/heatMapMax决定了在给定的一天中显示为全色的事件的数量。
  heatMapMax = settings.heatMapMax
}

//dayColorEnable determines if the different color for the day names should be used/dayColorEnable决定是否应使用不同颜色的日名。
let dayColorEnable = settings.dayColorEnable

//dayColor determines the color of the day names to be used/dayColor决定了要使用的日名的颜色。
let dayColor
...
//saturdaySundayHighlight 启用周六和周日的高亮显示（整个一天的颜色，而不仅仅是文本。
...
//satSunHighlightColor是高亮显示星期六和星期日全天的颜色。
    ...
    for (let j = 0; j < month[i].length; j += 1) {
      ...
      if ((month[i][j] === date.getDate().toString()) && !prevMonth && !nextMonth) {
        ...
        addWidgetTextLine(dateStackUp, `${month[i][j]}`,
        {
          ...
          font: Font.mediumSystemFont(10),//字重
          align: "center",
        });
      }
